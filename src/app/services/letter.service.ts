...
export class LetterService {
  ...
  public setLetter(oLetter: Letter) {
    ...
    const sUrl = this.gr.sUrlGlobal+'Correspondence/' + this._letter.id;
  ...
  getUserName(id_user_from: number, id_user_to: number) {
    const sUrl = this.gr.sUrlGlobal+'UserTable?id=' + id_user_from + '&id=' + id_user_to;
  ...
  getAnyLetter(id_letter: number) {
    const sUrl = this.gr.sUrlGlobal+'Correspondence?id=' + id_letter;
    return this.http.get(sUrl);
  }
  ...
  getThreadLetter(id_cv, id_vc: number) {
    const sUrl = this.gr.sUrlGlobal+'Correspondence?letter.id_cv=' + id_cv + '&letter.id_vc=' + id_vc;
    return this.http.get(sUrl);
  }
