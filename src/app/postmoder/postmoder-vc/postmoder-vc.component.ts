...
@Component({
  selector: 'app-postmoder-vc',
  templateUrl: './postmoder-vc.component.html',
  styleUrls: ['./postmoder-vc.component.css']
})
export class PostmoderVcComponent implements OnInit, OnDestroy {

  public vcList: any = [];

  public sVacancyShortTitle: string = '';
  public sVacancyDescription: string = '';
  public sConditions: string = '';
  public sJobFunction: string = '';
  public sVacancyRequirements: string = '';
  ...
  public sbDeleteVc: Subscription;
  public sApplyAllVc: Subscription;
  public sbscrDeleteAll: Subscription;
  public sbsctStartEv: Subscription;
  ...
  ngOnInit() {
    ...
    this.sbsctStartEv = this.checkpostservice.getCheckPostVc().subscribe(value =>
    {
      if (value !== undefined && value !== null)  {
        (value as any[]).forEach( (VC, index) => {
  ...
  ulClick($event, item: any, i: number) {
    if ($event.target.id !== "BTN" && $event.target.id !== "BTNDELETEALL") {
      ...
      if (item !== null && item != undefined) {
        ...
        this.sVacancyShortTitle = '';
        this.sVacancyDescription = '';
        this.sConditions = '';
        this.sJobFunction = '';
        this.sVacancyRequirements = '';
        this.sUserName = '';
        this.sEMail = '';
        this.sAvatar_Name = '';
        this.sZip = '';
        ...
        if (item.VacancyShortTitle !== undefined && item.VacancyShortTitle !==  null) this.sVacancyShortTitle =item.VacancyShortTitle;
        if (item.VacancyDescription !== undefined && item.VacancyDescription !==  null) this.sVacancyDescription = item.VacancyDescription;
        if (item.Conditions !== undefined && item.Conditions !==  null) this.sConditions =item.Conditions;
        if (item.JobFunction !== undefined &&  item.JobFunction !==  null) this.sJobFunction = item.JobFunction;
        if (item.VacancyRequirements !== undefined && item.VacancyRequirements !==  null) this.sVacancyRequirements = item.VacancyRequirements;
  ...
  DeleteElement($event, item: any, i: number) {
    ...
    this.sbDeleteVc = this.checkpostservice.setDeleteVc(item.id).subscribe( value=> {
        this.RouterReload();
      },
      err => console.log('при  удалении VС возникла нештатная ситуация ', err));
  }
  ...
  applyAll() {
    ...
    this.sApplyAllVc = this.checkpostservice.setApplyAllVc().subscribe( value=> {
        this.RouterReload();
      },
      err => console.log('при приеме всех VС возникла нештатная ситуация ', err));
  }
  ...
  ngOnDestroy() {
    if (typeof  this.sbDeleteVc !== 'undefined') {
      this.sbDeleteVc.unsubscribe();
    }

    if (typeof  this.sApplyAllVc !== 'undefined') {
      this.sApplyAllVc.unsubscribe();
    }
    ...
    if (typeof  this.sbsctStartEv !== 'undefined') {
      this.sbsctStartEv.unsubscribe();
    }
